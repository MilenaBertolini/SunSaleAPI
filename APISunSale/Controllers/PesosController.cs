...
using MainViewModel = Domain.ViewModel.PesosViewModel;
using MainEntity = Domain.Entities.Pesos;
using Service = Application.Interface.Services.IPesosService;
...
namespace APISunSale.Controllers
{
    ...
    {
        private readonly ILogger<PesosController> _logger;
        private readonly Service _service;
        private readonly IMapper _mapper;
        private readonly LoggerService _loggerService;

        public PesosController(ILogger<PesosController> logger, Service service, IMapper mapper, LoggerService loggerService)
        {
            _logger = logger;
            _service = service;
            _mapper = mapper;
        ...
        [HttpGet("getByCurso")]
        [AllowAnonymous]
        public async Task<ResponseBase<List<MainViewModel>>> GetByCurso(int curso)
        {
            try
            {
                var result = await _service.GetByCurso(curso);
                var response = _mapper.Map<List<MainViewModel>>(result);
                return new ResponseBase<List<MainViewModel>>()
                {
                    Message = "List created",
        ...
        [HttpGet("getCursos")]
        [AllowAnonymous]
        public async Task<ResponseBase<List<OpcaoCodigoNome>>> getCursos()
        {
            try
            {
                var result = _service.GetPesos();
