...
namespace Application.Interface.Services
{
    public interface IQuestoesService : IDisposable
    {
        ...
        Task<Tuple<IEnumerable<Main>, int>> GetAllPagged(int page, int quantity, int user, bool includeAnexos, string subject, string bancas, string provas, string materias, string tipos, int? codigoProva, TipoQuestoes? tipo, bool? randon, int? id, int? avaliacao);
