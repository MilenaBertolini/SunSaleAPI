...
{
    public class VerificacaoUsuarioRepository : RepositoryBase<Main>, IRepository
    {
        private static readonly string includes = "";

        public VerificacaoUsuarioRepository(DataContext dataContext) : base(dataContext)
        {
        }
        ...
        {
            var query = GetQueryable().Where(p => p.GuidText.Equals(guid));
            GetIncludes(includes).ToList().ForEach(p => query = query.Include(p));
