...
namespace Application.Implementation.Services
{
    public class ProvaService : IService
    {
        ...
        public async Task<string> CriaDocumentoProva(int codigo)
        {
            ...
            builder.AppendLine($"                                Prova: <b>{prova.NomeProva}</b>");
            builder.AppendLine($"                            </font>");
            builder.AppendLine($"                            <font size=\"-2\">");
            builder.AppendLine($"                                <br>Banca: <b>{prova.Banca}</b>");
            builder.AppendLine($"                                <br>Tipo: <b>{prova.TipoProva}</b>");
            builder.AppendLine($"                                <br>Data/Ano: <b>{prova.DataAplicacao}</b>");
            builder.AppendLine($"                            </font>");
            builder.AppendLine($"                        </td>");
            builder.AppendLine($"                        <td align=\"right\">");
            builder.AppendLine($"                            <font size=\"-1\">");
        ...
        public async Task<string> CriaDocumentoGabarito(int codigo)
        {
            Prova prova = await GetById(codigo);
            var questoes = await _serviceQuestoes.GetQuestoesByProva(prova.Codigo);
            ...
            builder.AppendLine($"    </style>");
            builder.AppendLine($"    <meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge\">");
            builder.AppendLine($"    <title>Gabarito - {prova.NomeProva}</title>");
            builder.AppendLine($"    <style type=\"text/css\" nonce=\"\">");
            builder.AppendLine(@"        body,
                                        td {
                                            font-size: 13px;
            ...
            builder.AppendLine($"                        <td> ");
            builder.AppendLine($"                            <font size=\"+1\">");
            builder.AppendLine($"                                Gabarito Prova: <b>{prova.NomeProva}</b>");
            builder.AppendLine($"                            </font>");
            builder.AppendLine($"                            <font size=\"-2\">");
            builder.AppendLine($"                                <br>Banca: <b>{prova.Banca}</b>");
            builder.AppendLine($"                                <br>Tipo: <b>{prova.TipoProva}</b>");
            ...
            {
                builder.AppendLine($"                   <div class=\"resposta\">");
                questao.RespostasQuestoes?.ToList().Where(q => q.Certa.Equals("1"))?.ToList()?.ForEach(resposta =>
                {

                    builder.AppendLine($"                       <h4>");
                    builder.AppendLine($"                           Questão {questao.NumeroQuestao} - Resposta certa: ");
